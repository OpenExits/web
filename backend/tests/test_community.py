...
from openexits_panel.models import ObjectConfirmation

OBJECT_ID = "01J9Y0AAAAAAAAAAAAAAAAAAAA"  # the seeded synthetic Héron object


def test_comment_thread_and_soft_delete(commons_app, contributor):
    client, headers = contributor
    r = client.post(f"/api/v1/objects/{OBJECT_ID}/comments", headers=headers,
                    json={"body": "Fil de discussion fictif — approche dégagée hier."})
    assert r.status_code == 201
    cid = r.get_json()["id"]

    community = commons_app.test_client().get(
        "/api/v1/public/objects/fr/pointe-du-heron/community").get_json()
    assert community["object_id"] == OBJECT_ID
    ...
    with commons_app.app_context():  # ...but survives as a soft-deleted row
        db = db_session()
        try:
            from openexits_panel.models import ObjectComment
            row = db.get(ObjectComment, cid)
            assert row is not None and row.deleted_at is not None
        finally:
            db.close()


def test_empty_comment_and_unknown_object_rejected(contributor):
    client, headers = contributor
    assert client.post(f"/api/v1/objects/{OBJECT_ID}/comments", headers=headers,
                       json={"body": "  "}).status_code == 422
    assert client.post("/api/v1/objects/01J9ZZZZZZZZZZZZZZZZZZZZZZ/comments",
                       headers=headers, json={"body": "x"}).status_code == 404


def test_confirm_upserts_and_rolling_window(commons_app, contributor):
    client, headers = contributor
    r1 = client.post(f"/api/v1/objects/{OBJECT_ID}/confirm", headers=headers)
    assert r1.status_code == 201
    assert r1.get_json()["confirmations_12mo"] == 1

    r2 = client.post(f"/api/v1/objects/{OBJECT_ID}/confirm", headers=headers)
    ...
    with commons_app.app_context():
        ...
        try:
            ...
            db.add(ghost)
            db.flush()
            db.add(ObjectConfirmation(object_id=OBJECT_ID, user_id=ghost.id,
...
def test_report_requires_category(contributor):
    client, headers = contributor
    bad = client.post(f"/api/v1/objects/{OBJECT_ID}/report", headers=headers,
                      json={"body": "quelque chose ne va pas"})
    assert bad.status_code == 422
    assert "categories" in bad.get_json()

    ok = client.post(f"/api/v1/objects/{OBJECT_ID}/report", headers=headers,
